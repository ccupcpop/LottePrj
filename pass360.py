...
def analyze_lottery(days_back=120, use_occurrence_limit=False, max_occurrences=5, threshold=1, validate_position=False):
    """
    分析539彩票拖牌統計
    ...
    # 取得最後7筆
    last_7_indices = list(range(len(df) - 7, len(df)))  # 獲取最後7筆在df中的真實index
    last_7 = df.iloc[last_7_indices].reset_index(drop=True)
    ...
    output_lines.append("最後7筆數據作為觸發條件：")
    output_lines.append(header)
    
    for i in range(6, -1, -1):  # 從最後1筆到倒數第7筆
        row = last_7.iloc[i]
        line = f"倒數第{7-i}筆 ({row['日期'].strftime('%Y/%m/%d')} 期{row['期號']}): 排序1={int(row['排序1'])}, 排序2={int(row['排序2'])}, 排序3={int(row['排序3'])}, 排序4={int(row['排序4'])}, 排序5={int(row['排序5'])}"
    ...
    # 對每個觸發條件進行分析（從最後1筆開始）
    for trigger_idx in range(6, -1, -1):
        trigger_row = last_7.iloc[trigger_idx]
        trigger_global_idx = last_7_indices[trigger_idx]  # 獲取在完整df中的真實index
        periods_ahead = 7 - trigger_idx  # 最後1筆找下1期，倒數第2筆找下2期...
        
        search_method = f"前{max_occurrences}次出現" if use_occurrence_limit else f"過去{days_back}筆"
        
        output_lines.append(header)
        output_lines.append(f"觸發條件 #{7-trigger_idx} (倒數第{7-trigger_idx}筆)")
        ...
        for pos in range(1, 6):
            ...
            search_end = max(0, trigger_global_idx - days_back)  # 限制搜尋範圍
    ...
    # 寫入檔案
    last_date = last_7.iloc[-1]['日期'].strftime('%Y%m%d')
    ...
    filename = f"{last_date}_號碼統計_{search_suffix}_門檻{threshold}{validate_suffix}.txt"
...
def main():
    ...
    USE_OCCURRENCE_LIMIT = False  # True=只統計前N次出現，False=統計天數內所有
    MAX_OCCURRENCES = 5  # 當USE_OCCURRENCE_LIMIT=True時，限制統計的出現次數
